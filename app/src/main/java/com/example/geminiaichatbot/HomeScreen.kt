...
fun AppContent(viewModel: HomeViewModel = androidx.lifecycle.viewmodel.compose.viewModel()) {
    ...
    HomeScreen(uiState = appUiState.value) { inputText, selectedItems ->
        coroutibeScope.launch {
            ...
            viewModel.question(userInput = inputText, selectedImages = bitmaps)
        }
    }
}
...
fun HomeScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "GeminiAi Chatbot") },
            ...
            Column {
                Row(modifier = Modifier.padding(vertical = 16.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    IconButton(
                        onClick = {
                        pickMediaLaucher.launch(PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.ImageAndVideo))
                    ...
                    OutlinedTextField(value = userQues,
                        onValueChange = {
                            userQues = it
                        }, placeholder = { Text(text = "Upload Image and task question") }
                    )
